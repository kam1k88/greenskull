...
# -------------------------------------------------------------
# 1.5. ВЕСА ПО СВЕЖЕСТИ (Step 2)
# -------------------------------------------------------------
def freshness_weight(M_values, M_last=88, tau_w=20.0):
    """
    Свежие вехи весят больше.
    M_last — последняя известная веха.
    tau_w — характерное время затухания.
    tau_w=inf → все веса 1.0 (baseline).
    """
    if np.isinf(tau_w):
        return np.ones_like(M_values, dtype=float)
    return np.exp(-(M_last - M_values) / tau_w)
...
def mc_fit_cpu(x, y, model_func_np, p0, phase_idx, poly_idx,
               ridge_idx=None, ridge_lambda=0.0, weights=None,
               n_iter=N_MC, eps=EPS, label=""):
    if weights is not None:
        sqrt_w = np.sqrt(np.asarray(weights, float).ravel())
    else:
        sqrt_w = None
...
def forecast(data, milestones, target_M,
             ...
             n_yearly=12, n_iter_mc=N_MC, tau=60.0, tau_w=float('inf'),
    ...
    for it in range(max_iter):
        ...
        # веса по свежести; anchor (последняя точка) всегда 1.0
        w_m2 = freshness_weight(m2_x_aug[:-1], M_last=last_M, tau_w=tau_w)
        w_m2 = np.concatenate([w_m2, [1.0]])
        ...
        # веса по свежести (используем те же M_values, что в M2)
        w_m3 = freshness_weight(mf['M'].values, M_last=last_M, tau_w=tau_w)
        w_m3 = np.concatenate([w_m3, [1.0]])
...
# -------------------------------------------------------------
# -------------------------------------------------------------
# 10. GRID SEARCH ПО TAU_W (Step 2)
# -------------------------------------------------------------
def grid_search_tau_w(milestones, data, ks, horizons, tau_ws,
                      n_yearly=12, n_iter_mc=100, tau=60.0):
    results = []
    for tw in tau_ws:
        tag = "inf (baseline)" if np.isinf(tw) else f"{tw}"
        print(f"\n=== tau_w = {tag} ===")
        df = backtest_multihorizon(milestones, data, ks, horizons,
                                   n_yearly=n_yearly, n_iter_mc=n_iter_mc,
                                   tau=tau, tau_w=tw, verbose=False)
        if len(df) == 0:
            print("  empty")
            continue
        m = metrics(df['error_days'].values)
        results.append({'tau_w': tw, **m})
        print(f"  n={m['n']} bias={m['bias']:+.2f} sigma={m['sigma']:.2f} "
              f"MAE={m['MAE']:.2f} RMSE={m['RMSE']:.2f} max={m['max_abs']:.2f}")
    return pd.DataFrame(results)
...
if __name__ == "__main__":
    ...
    # ---------- Baseline (Step 1) ----------
    print("\n" + "=" * 70)
    print("BASELINE (tau_w = inf, no freshness weights)")
    print("=" * 70)
    df_base = backtest_multihorizon(ms, data, ks, horizons,
                                    n_yearly=12, n_iter_mc=100,
                                    tau=60.0, tau_w=float('inf'),
                                    verbose=False)
    if len(df_base) > 0:
        m = metrics(df_base['error_days'].values)
        print(f"  n={m['n']} bias={m['bias']:+.2f} sigma={m['sigma']:.2f} "
              f"MAE={m['MAE']:.2f} RMSE={m['RMSE']:.2f} max={m['max_abs']:.2f}")
        MAE_baseline = m['MAE']
    else:
    ...
    # ---------- Grid search tau_w ----------
    print("\n" + "=" * 70)
    print("GRID SEARCH OVER TAU_W")
    print("=" * 70)
    tau_ws = [10.0, 20.0, 30.0, 50.0, 80.0, float('inf')]
    df_grid = grid_search_tau_w(ms, data, ks, horizons, tau_ws,
                                n_yearly=12, n_iter_mc=100, tau=60.0)
    ...
    if len(df_grid) > 0:
        ...
        # лучший без inf
        df_finite = df_grid[~df_grid['tau_w'].apply(np.isinf)]
        if len(df_finite) > 0:
            best = df_finite.loc[df_finite['MAE'].idxmin()]
            print(f"\nBest finite tau_w: {best['tau_w']} "
                  f"(MAE={best['MAE']:.3f}, bias={best['bias']:+.2f}, "
                  f"sigma={best['sigma']:.2f})")
            improvement = MAE_baseline - best['MAE']
            print(f"Improvement over baseline: {improvement:+.3f} days")
            if improvement >= 0.2:
                print(">>> INCLUDED (>=0.2 days improvement)")
                best_tau_w = float(best['tau_w'])
            else:
                print(">>> REJECTED (<0.2 days improvement)")
                best_tau_w = float('inf')
        else:
            best_tau_w = float('inf')
    else:
        best_tau_w = float('inf')
    ...
    # ---------- Финальный прогноз ----------
    print("\n" + "=" * 70)
    print(f"FINAL FORECAST 89M (tau_w = {best_tau_w})")
    print("=" * 70)
    res89 = forecast(data, ms, target_M=89, n_yearly=12, n_iter_mc=N_MC,
                     tau=60.0, tau_w=best_tau_w, verbose=True)
    if res89:
        print(f"\nDate 89M: {res89['date']}")
        print(f"dt=({res89['dt_m1']:.2f}, {res89['dt_m2']:.2f}, "
              f"{res89['dt_m3']:.2f})")
